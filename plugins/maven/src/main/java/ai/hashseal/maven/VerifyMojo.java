...
/**
 * Run {@code hashseal verify} against an integrity bundle.
 *
 * Copyright (c) 2026 MonkeyKing.dev
 */
@Mojo(name = "verify", defaultPhase = LifecyclePhase.VERIFY, requiresProject = false, threadSafe = true)
public class VerifyMojo extends AbstractMojo {
  ...
  public void execute() throws MojoExecutionException, MojoFailureException {
    if (skip) {
      getLog().info("hashseal:verify skipped (hashseal.skip=true)");
      return;
    }
    ...
    try {
      ...
      if (code != 0) {
        throw new MojoFailureException(
            "hashseal verify failed with exit code "
                + code
